...
class SimIn():
    ...
    def __init__(self):
        ...
        self.TPD = False  # temperature programmed desorption (TPD) mode flag
        self.TPD_start = None
        self.TPD_ramp = None
        self.T = None
        self.P = None
...
class ClusterIn(object):
    ...
    def ReadIn(self, fldr):
        ...
        RawTxt = _ReadWithoutBlankLines(os.path.join(fldr, self.fname),
                                        CommentLines=False)
        nLines = len(RawTxt)
        ...
        for j in range(nClusters):
            ...
            self.cluster_list[j].variant_list = [cluster_variant() for
                                                 k in range(n_variants)]

            # Loop through all variants for this cluster
            for k in range(n_variants):
                for i in range(variantInd[k, 0], variantInd[k, 1]):
                    if RawTxt[i].split()[0] == 'variant':
                        self.cluster_list[j].variant_list[k].name =\
                            RawTxt[i].split()[1]
                    elif RawTxt[i].split()[0] == 'site_types':
                        self.cluster_list[j].variant_list[k].site_types =\
                            RawTxt[i].split()[1:]
                    elif RawTxt[i].split()[0] == 'graph_multiplicity':
                        self.cluster_list[j].variant_list[k].\
                            graph_multiplicity = int(RawTxt[i].split()[1])
                    elif RawTxt[i].split()[0] == 'cluster_eng':
                        self.cluster_list[j].variant_list[k].cluster_eng =\
    ...
    def WriteIn(self, fldr):
        ...
        with open(os.path.join(fldr, self.fname), 'w') as txt:
            ...
            for clustr in self.cluster_list:
                ...
                if clustr.neighboring is not None:  # None when there
                                                    # is a point cluster
...
class MechanismIn(object):
    ...
    def ReadIn(self, fldr):
        ...
        if StiffCorrLine != -1:
            scaledown_factor_list = [np.float(i) for i in
                                     RawTxt[StiffCorrLine+2].
                                     split(':')[1].split()]
        ...
        for j in range(n_rxns):
            ...
            if n_variants == 0:     # There are no variants,
                                    # just one version of the reaction

                # raise NameError('No variants for reaction: ' +
                # self.rxn_list[j].name)
                n_variants = 1
            ...
            self.rxn_list[j].variant_list = [rxn_variant() for i in
                                             range(n_variants)]
            ...
            for k in range(n_variants):
                ...
                for i in range(variantInd[k, 0], variantInd[k, 1]):
                    if RawTxt[i].split()[0] == 'variant':
                        ...
                        self.rxn_list[j].variant_list[k].site_types =\
                            RawTxt[i].split()[1:]
                    elif RawTxt[i].split()[0] == 'pre_expon':
                        self.rxn_list[j].variant_list[k].pre_expon =\
                            float(RawTxt[i].split()[1])
                    elif RawTxt[i].split()[0] == 'pe_ratio':
                        self.rxn_list[j].variant_list[k].pe_ratio =\
                            float(RawTxt[i].split()[1])
                    elif RawTxt[i].split()[0] == 'activ_eng':
                        self.rxn_list[j].variant_list[k].activ_eng =\
                            float(RawTxt[i].split()[1])
                    elif RawTxt[i].split()[0] == 'prox_factor':
                        self.rxn_list[j].variant_list[k].prox_factor =\
                            float(RawTxt[i].split()[1])
                ...
                if StiffCorrLine != -1:
                    self.rxn_list[j].variant_list[k].scaledown_factor =\
                        scaledown_factor_list[all_rxn_ind]
                all_rxn_ind += 1
    ...
    def WriteIn(self, fldr):
        ...
        with open(os.path.join(fldr, self.fname), 'w') as txt:
            ...
            for rxn in self.rxn_list:
                ...
                if rxn.neighboring is not None:
                    txt.write('  neighboring')
                    for j in range(0, len(rxn.neighboring)):
                        txt.write(' ' + rxn.neighboring[j])
                    txt.write('\n')

                if rxn.gas_reacs_prods is not None:
                ...
                for rxn_var in rxn.variant_list:
                    ...
                    # Write pre-exponential factor. Add comment if
                    # it has been rescaled
                    if rxn_var.scaledown_factor == 1.0:     # reaction has not
                                                            # been rescaled
                        txt.write('    {:25}{:.5e}\n'.format('pre_expon',
                                  rxn_var.pre_expon))
...
class StateIn(object):
    ...
    def __init__(self):
        ...
        self.Type = None  # None: No state_input.dat, StateInput:
                          # has read state_input.dat, history:
                          # read from history file previously
        self.Struct = None
    ...
    def ReadOut(self, fldr, surf_spec_names, gas_spec_names):
        ...
        for i in range(RxnStartLine, RxnStartLine + self.nRxn):
            ...
            for j in range(0, len(RxnStrList)):
                ...
                if _re.search(r'\(', RxnStrList[j]):
                    SurfIden = _re.sub(r'\([^)]*\)', '', RxnStrList[j])
                    if SurfIden != '*':
                        SurfInd = [k for k in
                                   range(n_surf)
                                   if SurfIden == surf_spec_names[k]][0]
                        nu[SurfInd] += Sign
...
def Read_propensities(path, nRxn):
    '''
    Read propenisty data from output files. The initial time point: At t = 0 or
    after 0 events is wrong because it is recorded before the first
    propensities are calculated. Instead, they are erroneously all zeros and
    should not be used for averaging. The rest are correct.

    :param Mode: 0 - Read Prop_output.bin, instantaneous propensities
        1 - Read PropCounter_output.bin, time integrated propensities
        used for accurate time averages
...
def Read_time_integrated_propensities(path, nRxn):
    '''
    Read propenisty data from output files. The initial time point: At t = 0
    or after 0 events is wrong because it is recorded before the first
    propensities are calculated. Instead, they are erroneously all zeros and
    should not be used for averaging. The rest are correct.

    :param Mode: 0 - Read Prop_output.bin, instantaneous propensities
        1 - Read PropCounter_output.bin, time integrated propensities used
        for accurate time averages
    ...
    if os.path.isfile(os.path.join(path, 'PropCounter_output.bin')):
        ...
        dt = np.dtype(np.float64)
        virtual_arr = np.memmap(os.path.join(path, 'PropCounter_output.bin'),
                                dt, "r")
        nNum = virtual_arr.shape[0]
        nNum = nNum - (nNum % nRxn)
        virtual_arr = virtual_arr[:nNum]
...
def Read_trajectory_derivatives(path, nRxn):
    '''
    Read SA_output.bin - get trajectory derivatives for use in likelihood
...
def Read_time_integrated_species(path, n_surf_specs):
    ...
    if os.path.isfile(os.path.join(path, 'IntegSpec_output.bin')):
        dt = np.dtype(np.float64)
        virtual_arr = np.memmap(os.path.join(path, 'IntegSpec_output.bin'),
                                dt, "r")
        nNum = virtual_arr.shape[0]
        nNum = nNum - (nNum % n_surf_specs)
        virtual_arr = virtual_arr[:nNum]

        spec_num_int = np.reshape(virtual_arr,
                                  [nNum/n_surf_specs, n_surf_specs])
