...
filepath = os.path.join(Base_path, Input, 'Reference_set_info_ezgi.txt')
...
filepath = os.path.join(Base_path, 'Input', 'Tobe_Referenced_ezgi.txt')
[lines, dict] = _thermo.DFTFileRead(filepath)
T_target = []
for s in lines[3:]:
...
'''
filepath = os.path.join(Base_path, Input, 'Surface_energy_data_ezgi.txt')
[lines, dict] = _thermo.DFTFileRead(filepath)
T_surface = []
for s in lines[3:]:
